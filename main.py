...
def start_survey():
    ...
    resp = VoiceResponse()
    resp.say('你好,呢度係香港大學問卷調查,唔知你有冇時間幫手做份問卷呢?',language="zh-HK")
    gather = Gather(input='speech', action="/survey/"+record_id+"/1", language='yue-Hant-HK', method='POST', speechTimeout='auto')
    resp.append(gather)
...
def survery(record_id,question_id):
    ...
    for item in data:
        if item['record_id'] == record_id:
            item['question'].append({"question_id":question_id,"answer":choice})

    print("question_id:"+question_id)
    print("ans:" + choice)
    next_question = {1: '請問你叫咩名?',
                     2: '你今個晏晝食咗啲乜嘢啊?',
                     3: '大約消費幾多?',
                     4: '係邊度食啊?'}[int(question_id)]
    resp.say(next_question, language="zh-HK")

    if new_question_id == 5:
        gather = Gather(input='speech', action='/end_survey/'+record_id, language='yue-Hant-HK', method='POST',
                        speechTimeout='auto')
    else:
        gather = Gather(input='speech', action='/survey/'+record_id+'/'+str(new_question_id), language='yue-Hant-HK', method='POST', speechTimeout='auto')
